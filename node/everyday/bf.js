...
x('http://txly2.net/bf', 'tbody tr', {
    ...
    (function(err, audio) {
        ...
        download(audio.downUrl).then(data => {
            ...
            audio.path = "https://rawcdn.githack.com/quiet324/LiangYouRadioResource/" + commitTag + "/bf/"+ fileName;           
            audio.id = 1000000 + parseInt(audio.time.substring(2), 10);

            fs.writeFileSync("./bf.json", JSON.stringify(audio, null, '\t'));
            ...
            if (shell.exec('git tag bf' + audio.time).code !== 0) {
